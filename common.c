...
void read_fits_float(const char *filename, float **data, long *N, long *P, int *xa, int *ya, int *naxis_out) {
    ...
    *data = (float *)malloc(total_elements * sizeof(float));
    if (*data == NULL) {
        fprintf(stderr, "Error: Memory allocation failed for %ld floats.\n", total_elements);
        exit(1);
    }

    long fpixel[3] = {1, 1, 1};
    fits_read_pix(fptr, TFLOAT, fpixel, total_elements, NULL, *data, NULL, &status);
    CHECK_STATUS(status);
...
void write_fits_3d_float(const char *filename, float *data, int xa, int ya, int z_dim) {
    ...
    long fpixel[3] = {1, 1, 1};
    fits_write_pix(fptr, TFLOAT, fpixel, total_elements, data, &status);
    CHECK_STATUS(status);
